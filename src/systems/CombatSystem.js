...
export class CombatSystem {
    ...
    trails = [];
    timerSystem;
    setEntities(players, boomerangs, explosions, trails, timerSystem) {
    ...
    update() {
        ...
        for (const boomerang of this.boomerangs) {
            ...
            for (const player of this.players) {
                if (boomerang.ownerId === player.id)
                    continue;
        ...
        for (const explosion of this.explosions) {
            for (const player of this.players) {
                if (explosion.ownerId === player.id)
                    continue;
                if (player.state === PlayerState.DEAD || explosion.hitIds.has(player.id))
                    continue;
                this.checkExplosionHit(explosion, player);
            }
        }
    }
    checkHit(boomerang, player) {
    ...
    checkExplosionHit(explosion, player) {
