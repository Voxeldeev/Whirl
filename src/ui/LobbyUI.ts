...
interface CardData { id: ModifierId; title: string; desc: string; type: 'A' | 'B'; }
...
export class LobbyUI {
    public onReadyToStart?: (loadout: ModifierId[], color: string) => void;
    
    private selectedColor: string = '#00a8ff';
    private slotA: CardData | null = null;
    private slotB1: CardData | null = null;
    private slotB2: CardData | null = null;
    ...
    private initializeUI() {
        ...
        colors.forEach((color, i) => {
            ...
            swatch.addEventListener('click', (e) => {
                ...
                (document.querySelector('.player-indicator') as HTMLElement).style.borderLeftColor = color;
            });
        ...
        document.getElementById('btn-join')!.addEventListener('click', () => {
            const code = (document.getElementById('input-code') as HTMLInputElement).value.toUpperCase();
            if (code.length === 5) this.network.joinGame(code);
        });
    ...
    private tryEquip(card: CardData, el: HTMLElement) {
    ...
    private unequip(slotId: 'A' | 'B1' | 'B2') {
        let card: CardData | null = null;
        if (slotId === 'A') { card = this.slotA; this.slotA = null; this.clearSlot('slot-a', 'CLASS A'); }
        else if (slotId === 'B1') { card = this.slotB1; this.slotB1 = null; this.clearSlot('slot-b1', 'CLASS B'); }
        else if (slotId === 'B2') { card = this.slotB2; this.slotB2 = null; this.clearSlot('slot-b2', 'CLASS B'); }

        if (card) this.enableCardInGrid(card.id);
    }

    private updateSlot(slotId: string, card: CardData) {
        const slot = document.getElementById(slotId)!;
        slot.innerHTML = `<h3>${card.title}</h3><p style="font-size: 0.7rem; color: #aaa; margin:0;">${card.desc.replace('\n', '<br>')}</p>`;
    }

    private clearSlot(slotId: string, label: string) {
        document.getElementById(slotId)!.innerHTML = `<span class="slot-label">${label}</span>`;
    }

    private enableCardInGrid(id: ModifierId) {
        const index = CARDS.findIndex(c => c.id === id);
        if (index !== -1) document.getElementById('card-grid')!.children[index].classList.remove('disabled');
    }

    private bindNetworkEvents() {
        this.network.onReady = (roomId: string) => {
